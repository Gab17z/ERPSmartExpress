  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (ultimaCompra?.numero) {
        const match = ultimaCompra.numero.match(/(\d+)$/);
      ...
      for (const item of data.itens) {
        ...
        if (item.produto_id.startsWith('temp_')) {
          ...
          // CORREÇÃO: Produto existente - buscar estoque FRESCO do banco para evitar race condition
          const produtoFresco = await base44.entities.Produto.get(produtoId);
          if (produtoFresco) {
            const estoqueAtual = parseInt(produtoFresco.estoque_atual) || 0;
            const quantidadeEntrada = parseInt(item.quantidade) || 0;
            const novoEstoque = estoqueAtual + quantidadeEntrada;
            const novoCusto = parseFloat(item.preco_unitario) || produtoFresco.preco_custo || 0;
            
            await base44.entities.Produto.update(produtoFresco.id, {
            ...
            await base44.entities.MovimentacaoEstoque.create({
              tipo: "entrada",
              produto_id: produtoFresco.id,
              produto_nome: produtoFresco.nome,
