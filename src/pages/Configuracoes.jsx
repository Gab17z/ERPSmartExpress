      ...
      try {
        ...
        if (backup.configuracoes) {
          const mergedConfigs = {
            ...
            empresa: { ...configuracoesDefault.empresa, ...(backup.configuracoes.empresa || {}) },
            pdv: { ...configuracoesDefault.pdv, ...(backup.configuracoes.pdv || {}) },
            calculadora: { ...configuracoesDefault.calculadora, ...(backup.configuracoes.calculadora || {}) },
            os: {
              ...configuracoesDefault.os,
              ...(backup.configuracoes.os || {}),
              checklist_entrada: Array.isArray(backup.configuracoes.os?.checklist_entrada) ? backup.configuracoes.os.checklist_entrada : configuracoesDefault.os.checklist_entrada,
              checklist_finalizacao: Array.isArray(backup.configuracoes.os?.checklist_finalizacao) ? backup.configuracoes.os.checklist_finalizacao : configuracoesDefault.os.checklist_finalizacao,
            },
            seminovos: {
              ...configuracoesDefault.seminovos,
              ...(backup.configuracoes.seminovos || {}),
              acessorios_customizados: Array.isArray(backup.configuracoes.seminovos?.acessorios_customizados) ? backup.configuracoes.seminovos.acessorios_customizados : configuracoesDefault.seminovos.acessorios_customizados,
            },
            impressao: {
              ...configuracoesDefault.impressao,
              ...(backup.configuracoes.impressao || {})
            },
            monitor_cliente: { ...configuracoesDefault.monitor_cliente, ...(backup.configuracoes.monitor_cliente || {}) },
            notificacoes: { ...configuracoesDefault.notificacoes, ...(backup.configuracoes.notificacoes || {}) },
            sistema: { ...configuracoesDefault.sistema, ...(backup.configuracoes.sistema || {}) }
          };
        ...
        <TabsContent value="os">
          <Tabs defaultValue="geral" className="space-y-4">
            ...
            <TabsContent value="automacao">
              <Card className="border-none shadow-lg">
                <CardHeader><CardTitle>Automação e Notificações</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
            ...
            {/* ANTIGA Sub-aba: Editor Visual (BACKUP) */}
            <TabsContent value="editor-old">
              <div className="space-y-4" style={{ display: 'none' }}>
                {/* Layouts Rápidos */}
                <Card className="border-2 border-purple-200 bg-purple-50">
                  <CardHeader className="p-4 pb-2">
                    <CardTitle className="text-base text-purple-900">Templates Rápidos</CardTitle>
                    <p className="text-sm text-purple-700">Escolha um layout pré-definido para começar.</p>
                  </CardHeader>
                  <CardContent className="p-4 pt-0">
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
                      {templateOS.opcoes_layout.map((layout) => (
                        <Button
                          key={layout.id}
                          variant={templateOS.layout_ativo === layout.id ? "default" : "outline"}
                          size="sm"
                          onClick={() => aplicarLayoutPredefinido(layout.id)}
                          className={`flex flex-col h-auto py-3 text-xs justify-center items-center text-center ${templateOS.layout_ativo === layout.id ? "bg-blue-600 hover:bg-blue-700 text-white" : "border-slate-300 text-slate-700 hover:bg-slate-100"}`}
                        >
                          <div className="font-bold">{layout.nome}</div>
                          <div className="text-[10px] opacity-80 mt-1">{layout.descricao}</div>
                        </Button>
                      ))}
                    </div>
                  </CardContent>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                  ...
                  <div className="md:col-span-3 space-y-4">
                    <Card className="border-none shadow-lg">
                      <CardHeader className="p-4 pb-2">
                        <CardTitle className="text-base">Estrutura da OS</CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-2 max-h-[70vh] overflow-y-auto p-4 pt-0">
                        {templateOS.secoes.map((secao) => (
                          <div key={secao.id} className={`border rounded-lg p-3 ${!secao.ativo ? 'opacity-50 bg-slate-50' : 'bg-white'}`}>
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-2">
                                ...
                                />
                                <span className="text-sm font-semibold flex-1">{secao.nome}</span>
                              </div>
                            </div>
                            ...
                            {secao.ativo && (
                              <div className="ml-6 mt-2 space-y-2">
                                {secao.campos.map((campo) => (
                                  <div
                                    key={campo.id}
                                    className={`p-2 rounded cursor-pointer transition-colors text-xs flex items-center justify-between ${campoSelecionado?.secaoId === secao.id && campoSelecionado?.id === campo.id ? 'bg-blue-100 border border-blue-500' : 'hover:bg-slate-50 border border-transparent'}`}
                                    onClick={() => setCampoSelecionado({ ...campo, secaoId: secao.id })}
                                  >
                                    <div className="flex items-center gap-2">
                                      <input
                                        type="checkbox"
                                        id={`checkbox-${secao.id}-${campo.id}`}
                                        checked={campo.visivel}
                                        onChange={(e) => {
                                          e.stopPropagation();
                                          toggleCampo(secao.id, campo.id);
                                        }}
                                        className="w-3.5 h-3.5"
                                      />
                                      <label htmlFor={`checkbox-${secao.id}-${campo.id}`} className="cursor-pointer">{campo.label}</label>
                                    </div>
                                    <Badge variant="secondary" className="text-[10px] px-1 py-0.5">{campo.tipo}</Badge>
                                  </div>
                                ))}
                              </div>
                            )}
                    ...
                    <Button onClick={handleSalvarTemplate} className="w-full bg-green-600 hover:bg-green-700">
                      <Save className="w-4 h-4 mr-2" />
                      Salvar Template
                    </Button>
                  </div>
                    ...
                    <Card className="border-none shadow-lg">
                      <CardHeader className="p-4 pb-2">
                        <CardTitle className="text-base flex items-center justify-between">
                          <span>Preview da Ordem de Serviço</span>
                          <Badge variant="secondary" className="text-xs">Edição ao Vivo</Badge>
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="p-4 pt-0">
                        <div className="bg-white border-2 border-slate-300 rounded-lg p-6 max-h-[70vh] overflow-y-auto" style={{ fontFamily: templateOS.estilo_global.fonte, backgroundColor: templateOS.estilo_global.cor_fundo || '#ffffff', color: templateOS.estilo_global.cor_texto || '#000000' }}>
                          {templateOS.secoes.filter(s => s.ativo).map((secao) => {
                            const camposVisiveis = secao.campos.filter(c => c.visivel);
                            if (camposVisiveis.length === 0) return null;

                            return (
                              <div
                                key={secao.id}
                                className={`mb-4 ${secao.borda ? 'border border-slate-300' : ''} rounded-lg p-4`}
                                style={{ backgroundColor: secao.cor_fundo || 'transparent' }}
                              >
                                <h3 className="font-bold mb-3 pb-2 border-b" style={{ fontSize: '1.2em' }}>{secao.nome}</h3>

                                <div className="grid grid-cols-2 gap-2"> {/* Responsive grid for fields */}
                                  {camposVisiveis.map((campo) => {
                                    ...
                                    const estiloTexto = {
                                      ...
                                      textAlign: campo.alinhamento || 'left',
                                    };

                                    if (campo.tipo === 'imagem') {
                                      return (
                                        <div key={campo.id} className="col-span-2 md:col-span-1" style={{ textAlign: campo.alinhamento || 'left' }}>
                                          <div className="inline-block w-20 h-20 bg-slate-200 rounded flex items-center justify-center">
                                    ...
                                    if (campo.tipo === 'checklist') {
                                      return (
                                        <div key={campo.id} className="col-span-2" style={estiloTexto}>
                                          <p className="font-semibold" style={{ fontSize: estiloTexto.fontSize }}>{campo.label}</p>
                                          <div className={`grid gap-1 grid-cols-${campo.colunas || 2}`} style={{ fontSize: estiloTexto.fontSize }}>
                                            <div>☐ Item Checklist 1</div>
                                            <div>☑ Item Checklist 2</div>
                                            <div>☐ Item Checklist 3</div>
                                            <div>☑ Item Checklist 4</div>
                                          </div>
                                        </div>
                                      );
                                    }
                                    ...
                                    if (campo.tipo === 'tabela') {
                                      return (
                                        <div key={campo.id} className="col-span-2" style={estiloTexto}>
                                          <p className="font-semibold" style={{ fontSize: estiloTexto.fontSize }}>{campo.label}</p>
                                          <table className="w-full border" style={{ fontSize: estiloTexto.fontSize }}>
                                            <thead className="bg-slate-100">
                                              <tr>
                                                <th className="p-1 border text-left">Descrição</th>
                                                <th className="p-1 border text-right">Valor</th>
                                              </tr>
                                            </thead>
                                            <tbody>
                                              <tr>
                                                <td className="p-1 border">Item Exemplo</td>
                                                <td className="p-1 border text-right">R$ 100,00</td>
                                              </tr>
                                            </tbody>
                                          </table>
                                        </div>
                                    ...
                                    if (campo.tipo === 'linha_assinatura') {
                                      return (
                                        <div key={campo.id} className="col-span-2 text-center mt-6">
                                          <div className="inline-block border-t-2 border-slate-800 w-64 pt-2" style={estiloTexto}>
                                            {campo.label}
                                          </div>
                                        </div>
                                      );
                                    ...
                                    if (campo.tipo === 'texto_longo') {
                                      return (
                                        <div key={campo.id} className="col-span-2">
                                          <p className="font-semibold" style={{ fontSize: estiloTexto.fontSize }}>{campo.label}:</p>
                                          <p style={estiloTexto}>
                                            Texto de exemplo do campo {campo.label.toLowerCase()}. Este é um campo de texto longo que pode conter múltiplas linhas.
                                          </p>
                                        </div>
                                      );
                                    }
                                    ...
                                    // Tipo texto normal
                                    return (
                                      <div key={campo.id} className="col-span-1" style={{ textAlign: campo.alinhamento || 'left' }}>
                                        <span className="font-semibold" style={{ fontSize: estiloTexto.fontSize }}>{campo.label}: </span>
                                        <span style={estiloTexto}>
                                          {campo.id === 'codigo_os' ? 'OS-00001' :
                                            campo.id === 'cliente_nome' ? 'Cliente Exemplo' :
                                              campo.id === 'aparelho_marca' ? 'Samsung' :
                                                campo.id === 'aparelho_modelo' ? 'Galaxy A32' :
                                                  campo.id === 'valor_total' ? 'R$ 350,00' :
                                                    'Valor de exemplo'}
                                        </span>
                                      </div>
                                    );
                  ...
                  <div className="md:col-span-3 space-y-4">
                    {campoSelecionado ? (
                      <Card className="border-2 border-blue-500 shadow-lg">
                        <CardHeader className="p-4 pb-2">
                          <CardTitle className="text-base">Editar Campo: {campoSelecionado.label}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-3 p-4 pt-0">
                          <div>
                            <Label className="text-xs">Tamanho da Fonte</Label>
                            <Select
                              value={campoSelecionado.tamanho || 'medio'}
                              onValueChange={(v) => {
                                editarCampo(campoSelecionado.secaoId, campoSelecionado.id, 'tamanho', v);
                                setCampoSelecionado({ ...campoSelecionado, tamanho: v });
                              }}
                            >
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <Switch
                              checked={campoSelecionado.negrito || false}
                              onCheckedChange={(c) => {
                                editarCampo(campoSelecionado.secaoId, campoSelecionado.id, 'negrito', c);
                                setCampoSelecionado({ ...campoSelecionado, negrito: c });
                              }}
                            />
                          </div>
                        </CardContent>
                      ...
                      <Card className="border-none shadow-lg">
                        <CardHeader className="p-4 pb-2">
                          <CardTitle className="text-base">Estilo Global</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4 p-4 pt-0">
                          <div>
                            <Label className="text-xs">Fonte</Label>
                            <Select
                              value={templateOS.estilo_global.fonte}
                              onValueChange={(v) => setTemplateOS(prev => ({
                                ...prev,
                                estilo_global: { ...prev.estilo_global, fonte: v }
                              }))}
                            >
                              <SelectTrigger className="text-xs">
                                <SelectValue />
                          ...
                          <div>
                            ...
                            <Input
                              value={templateOS.estilo_global.tamanho_fonte_base}
                              onChange={(e) => setTemplateOS(prev => ({
                                ...prev,
                                estilo_global: { ...prev.estilo_global, tamanho_fonte_base: e.target.value }
                              }))}
                              placeholder="10px"
                            />
                          </div>
                          ...
                          <div>
                            ...
                            <Select
                              value={templateOS.estilo_global.espacamento}
                              onValueChange={(v) => setTemplateOS(prev => ({
                                ...prev,
                                estilo_global: { ...prev.estilo_global, espacamento: v }
                              }))}
                            >
                              <SelectTrigger className="text-xs">
                                <SelectValue />
                          ...
                          <div className="flex items-center justify-between">
                            <Label className="text-xs">Bordas nas Seções</Label>
                            <Switch
                              checked={templateOS.estilo_global.borda_secoes}
                              onCheckedChange={(c) => setTemplateOS(prev => ({
                                ...prev,
                                estilo_global: { ...prev.estilo_global, borda_secoes: c }
                              }))}
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              {['#000000', '#1e293b', '#475569', '#64748b'].map(cor => (
                                <button
                                  key={cor}
                                  onClick={() => setTemplateOS(prev => ({
                                    ...prev,
                                    estilo_global: { ...prev.estilo_global, cor_texto: cor }
                                  }))}
                                  className="w-8 h-8 rounded border-2"
                                  style={{
                                    backgroundColor: cor,
                                    borderColor: templateOS.estilo_global.cor_texto === cor ? '#3b82f6' : '#ccc'
                                  }}
                                />
                              ))}
                            </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              {['#ffffff', '#f8fafc', '#f1f5f9', '#e2e8f0'].map(cor => (
                                <button
                                  key={cor}
                                  onClick={() => setTemplateOS(prev => ({
                                    ...prev,
                                    estilo_global: { ...prev.estilo_global, cor_fundo: cor }
                                  }))}
                                  className="w-8 h-8 rounded border-2"
                                  style={{
                                    backgroundColor: cor,
                                    borderColor: templateOS.estilo_global.cor_fundo === cor ? '#3b82f6' : '#ccc'
                                  }}
                                />
                              ))}
                            </div>
                    ...
                    <Card className="border-2 border-green-200 bg-green-50 shadow-lg">
                      <CardContent className="p-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-green-900 text-sm">Dicas de Uso:</p>
                            <ul className="text-xs text-green-800 mt-2 space-y-1">
                              <li>• Clique em um campo para editá-lo</li>
                              <li>• Use os switches para mostrar/ocultar</li>
                              <li>• Desative seções inteiras se não precisar</li>
                              <li>• Personalize cores, fontes e tamanhos</li>
                              <li>• Salve para aplicar na impressão</li>
                            </ul>
                          </div>
                        </div>
                      </CardContent>
              ...
              <Card className="border-none shadow-lg">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    <Label>Idioma</Label>
                    <Select
                      value={configuracoes.sistema?.idioma ?? "pt-BR"}
                      onValueChange={(v) => handleChange('sistema', 'idioma', v)}
                    >
