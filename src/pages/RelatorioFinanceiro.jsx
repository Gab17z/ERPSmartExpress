...
export default function RelatorioFinanceiro() {
  ...
  const contasRecebidasPeriodo = contasReceber.filter(c => c.situacao === 'pago' && c.data_recebimento && filtrarPorData(c.data_recebimento));
  const contasPagasPeriodo = contasPagar.filter(c => c.situacao === 'pago' && c.data_pagamento && filtrarPorData(c.data_pagamento));
  ...
  const receitaOutras = contasRecebidasPeriodo.reduce((sum, c) => sum + (parseFloat(c.valor_recebido || c.valor_total) || 0), 0);
  const receitaTotal = receitaVendas + receitaOutras + totalSuplementos;

  // CRÍTICO: Despesas = Contas Pagas + Comissões Pagas + Sangrias
  const despesasContasPagar = contasPagasPeriodo.reduce((sum, c) => sum + (parseFloat(c.valor_total) || 0), 0);
  ...
  // Pendências
  const contasReceberPendentes = contasReceber.filter(c => c.situacao !== 'pago').reduce((sum, c) => sum + ((parseFloat(c.valor_total) || 0) - (parseFloat(c.valor_recebido) || 0)), 0);
  const contasPagarPendentes = contasPagar.filter(c => c.situacao !== 'pago').reduce((sum, c) => sum + (parseFloat(c.valor_total) || 0), 0);

  // Contas vencidas
  const hojeDate = startOfDay(new Date());
  const contasReceberVencidas = contasReceber.filter(c => c.situacao !== 'pago' && new Date(c.data_vencimento) < hojeDate);
  const contasPagarVencidas = contasPagar.filter(c => c.situacao !== 'pago' && new Date(c.data_vencimento) < hojeDate);
  ...
  const despesasPorCategoria = contasPagasPeriodo.reduce((acc, c) => {
    ...
    acc[cat] = (acc[cat] || 0) + (parseFloat(c.valor_total) || 0);
    return acc;
  }, {});
  ...
  const exportarCSV = () => {
    ...
    const rows = [
      ...
      ...contasRecebidasPeriodo.map(c => [
        format(new Date(c.data_recebimento), 'dd/MM/yyyy'),
        'Receita',
        c.descricao,
        'Conta a Receber',
              ...
              <div>
                ...
                {contasReceberVencidas.length > 0 && (
                  <p className="text-sm text-red-700 mt-1">
                    {contasReceberVencidas.length} conta(s) a receber vencida(s) • Total: R$ {contasReceberVencidas.reduce((s, c) => s + ((parseFloat(c.valor_total) || 0) - (parseFloat(c.valor_recebido) || 0)), 0).toFixed(2)}
                  </p>
                )}
                {contasPagarVencidas.length > 0 && (
                  <p className="text-sm text-red-700 mt-1">
                    {contasPagarVencidas.length} conta(s) a pagar vencida(s) • Total: R$ {contasPagarVencidas.reduce((s, c) => s + (parseFloat(c.valor_total) || 0), 0).toFixed(2)}
                  </p>
                )}
              </div>
            </div>
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {contasReceber.filter(c => c.situacao !== 'pago').slice(0, 15).map(conta => (
                    <TableRow key={conta.id} className={new Date(conta.data_vencimento) < hojeDate ? 'bg-red-50' : ''}>
                      ...
                      <TableCell className="font-bold text-green-600">R$ {((conta.valor_total || 0) - (conta.valor_recebido || 0)).toFixed(2)}</TableCell>
                      <TableCell>
                        <Badge variant={conta.situacao === 'parcial' ? 'secondary' : 'destructive'}>
                          {conta.situacao}
                        </Badge>
                      </TableCell>
                    </TableRow>
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {contasPagar.filter(c => c.situacao !== 'pago').slice(0, 15).map(conta => (
                    <TableRow key={conta.id} className={new Date(conta.data_vencimento) < hojeDate ? 'bg-red-50' : ''}>
