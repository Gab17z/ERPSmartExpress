      ...
      if (orcamento.pecas) {
        orcamento.pecas.forEach(p => {
          ...
          if (p.produto_id) {
            ...
            const produtoData = produtos.find(pp => pp.id === p.produto_id);
            itensCarrinho.push({
              ...
              preco_custo: produtoData?.preco_custo || 0, // CRÍTICO: Registra o custo atual do produto no estoque
              desconto_item: 0,
    ...
    mutationFn: async (vendaData) => {
      ...
      try {
        if (caixaAberto && venda.pagamentos && venda.pagamentos.length > 0) {
          ...
          if (somaTotal > 0) {
            // CRÍTICO: Buscar saldo REAL do caixa no banco imediatamente antes de atualizar
            // para evitar que vendas em outras máquinas "atropelem" o saldo atual
            const caixasNoBanco = await base44.entities.Caixa.filter({ id: caixaAberto.id });
            const caixaReal = caixasNoBanco[0] || caixaAberto;

            const novoTotalEntradas = Math.round((parseFloat(caixaReal.total_entradas || 0) + somaTotal) * 100) / 100;
            const novoSaldoDinheiro = Math.round((parseFloat(caixaReal.saldo_dinheiro || 0) + somaDinheiro) * 100) / 100;
            const novoSaldoCartao = Math.round((parseFloat(caixaReal.saldo_cartao || 0) + somaCartao) * 100) / 100;
            const novoSaldoPix = Math.round((parseFloat(caixaReal.saldo_pix || 0) + somaPix) * 100) / 100;
            const novoSaldoFinal = Math.round((parseFloat(caixaReal.saldo_final || 0) + somaTotal) * 100) / 100;

            await base44.entities.Caixa.update(caixaAberto.id, {
              total_entradas: novoTotalEntradas,
              saldo_dinheiro: novoSaldoDinheiro,
              saldo_cartao: novoSaldoCartao,
              saldo_pix: novoSaldoPix,
...
Forma(s) de Pagamento: ${(venda.pagamentos || []).map(p => p.forma_pagamento).join(', ')}`
  ...
  const adicionarAoCarrinho = (produto) => {
    ...
    if (itemExistente) {
      ...
      setCarrinho([...carrinho, {
        ...
        preco_custo: produto.preco_custo || 0, // CRÍTICO: Registra o custo atual do produto
        desconto_item: 0,
  ...
  const finalizarVenda = async () => {
    ...
    if (pagamentoPix && pagamentoPix.valor > 0) {
      // CORREÇÃO DE SEGURANÇA: Ativar mutex ANTES de abrir o PIX
      // para evitar que o vendedor clique em "Finalizar" novamente
