...
export default function OrdensServico() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      ...
      let proximoNumero = 1;
      try {
        // Buscar maior número de OS existente para garantir sequência correta
        const todasOS = await base44.entities.OrdemServico.list();
        if (todasOS.length > 0) {
          const numeros = todasOS
            .map(os => parseInt(os.codigo_os?.replace('OS-', '') || '0'))
            .filter(n => !isNaN(n));
          if (numeros.length > 0) {
        ...
        // Atualizar ou criar configuração para manter sincronizado
        ...
        // Fallback: buscar maior número existente mesmo em caso de erro na config
        try {
          const todasOS = await base44.entities.OrdemServico.list();
          const numeros = todasOS
            .map(os => parseInt(os.codigo_os?.replace('OS-', '') || '0'))
