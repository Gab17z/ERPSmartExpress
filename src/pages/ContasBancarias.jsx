...
export default function ContasBancarias() {
  ...
  const transferenciaMutation = useMutation({
    mutationFn: async (data) => {
      // CRÍTICO: Validações de transferência bancária
      const contaOrigem = contas.find(c => c.id === data.conta_origem_id);
      const contaDestino = contas.find(c => c.id === data.conta_destino_id);
      ...
      // CRÍTICO: Validar e calcular saldos com segurança
      ...
      await base44.entities.ContaBancaria.update(contaOrigem.id, {
        saldo_atual: saldoOrigemAtual - valor
      });

      await base44.entities.ContaBancaria.update(contaDestino.id, {
        saldo_atual: saldoDestinoAtual + valor
      });
