...
export default function Relatorios() {
  ...
  // Mapear devoluções aprovadas por venda
  ...
  // CRÍTICO: Dedução das devoluções do faturamento total
  const faturamentoTotal = Math.round(vendasPeriodo.reduce((sum, v) => sum + Math.max(0, (v.valor_total || 0) - (devolucoesPorVenda[v.id] || 0)), 0) * 100) / 100;
  // Ticket médio: dividir pelo nº de vendas com valor líquido > 0 (excluir 100% devolvidas)
  const vendasComValor = vendasPeriodo.filter(v => Math.max(0, (v.valor_total || 0) - (devolucoesPorVenda[v.id] || 0)) > 0).length;
  const ticketMedio = vendasComValor > 0 ? Math.round((faturamentoTotal / vendasComValor) * 100) / 100 : 0;
  ...
  // Lucro estimado com dedução de devoluções
  const lucroTotal = Math.round(vendasPeriodo.reduce((sum, venda) => {
    ...
    const custoTotalBase = venda.itens?.reduce((itemSum, item) => {
      const custoUnidade = item.preco_custo !== undefined ? item.preco_custo : (produtos.find(p => p.id === item.produto_id)?.preco_custo || 0);
      return itemSum + (custoUnidade * item.quantidade);
    }, 0) || 0;
  ...
  const margemMedia = faturamentoTotal > 0 ? Math.round(((lucroTotal / faturamentoTotal) * 100) * 100) / 100 : 0;

  // Vendas por vendedor (incluindo comissões pagas e deduzindo devoluções)
  const vendasPorVendedor = {};
  vendasPeriodo.forEach(venda => {
    const vendedor = venda.vendedor_nome || "Sem vendedor";
    const valorOriginal = parseFloat(venda.valor_total) || 0;
    const valorDevolvido = devolucoesPorVenda[venda.id] || 0;
    const faturamentoLiquido = Math.max(0, valorOriginal - valorDevolvido);
    const percentualRetido = valorOriginal > 0 ? faturamentoLiquido / valorOriginal : 0;

    const custoTotalBase = venda.itens?.reduce((sum, item) => {
      const custoUnidade = item.preco_custo !== undefined ? item.preco_custo : (produtos.find(p => p.id === item.produto_id)?.preco_custo || 0);
      return sum + (custoUnidade * item.quantidade);
    }, 0) || 0;
    const custoRetido = custoTotalBase * percentualRetido;
    ...
    const lucroVenda = faturamentoLiquido - custoRetido - valorComissao;

    if (vendasPorVendedor[vendedor]) {
      vendasPorVendedor[vendedor].quantidade++;
      vendasPorVendedor[vendedor].valor += faturamentoLiquido;
  ...
  // CRÍTICO: Lucratividade com validação (deduzindo devoluções e incluindo comissões)
  const lucroDetalhado = vendasPeriodo.map(venda => {
    ...
    const custoTotalBase = venda.itens?.reduce((sum, item) => {
      const custoUnidade = item.preco_custo !== undefined ? item.preco_custo : (produtos.find(p => p.id === item.produto_id)?.preco_custo || 0);
      return sum + ((parseFloat(custoUnidade) || 0) * (parseInt(item.quantidade) || 0));
    }, 0) || 0;
    ...
    const custoTotalComComissao = custoRetido + valorComissao;
    const lucro = faturamentoLiquido - custoTotalComComissao;
  ...
  const exportarExcel = () => {
    const dados = vendasPeriodo.map(venda => ({
      ...
      'Valor Total': Math.max(0, (parseFloat(venda.valor_total) || 0) - (devolucoesPorVenda[venda.id] || 0)),
