...
export default function DRE() {
  ...
  const { data: contasReceber = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        return await base44.entities.ContaReceber.list();
      } catch {
        return [];
      }
    },
  ...
  const receitaVendas = vendasFinalizadas.reduce((sum, v) => sum + (parseFloat(v.valor_total) || 0), 0);
  
  const receitasRecebidas = contasReceber.filter(c => c.situacao === 'pago' && c.data_recebimento && filtrarPorData(c.data_recebimento)).reduce((sum, c) => sum + (parseFloat(c.valor_recebido) || 0), 0);
  ...
  const custoMercadorias = vendasFinalizadas.reduce((sum, venda) => {
    const custoVenda = venda.itens?.reduce((itemSum, item) => {
      ...
      return itemSum + ((parseFloat(produto?.preco_custo) || 0) * (parseInt(item.quantidade) || 0));
    }, 0) || 0;
  ...
  const despesasContasPagar = contasPagar.filter(c => c.situacao === 'pago' && c.data_pagamento && filtrarPorData(c.data_pagamento)).reduce((sum, c) => sum + (parseFloat(c.valor_total) || 0), 0);
