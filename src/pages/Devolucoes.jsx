      ...
      if (devolucao.tipo_reembolso !== 'troca') {
        try {
          ...
          if (caixasAbertos.length > 0) {
            ...
            // CRÍTICO: Re-buscar saldo REAL do caixa no banco para evitar condição de corrida
            const caixasBanco = await base44.entities.Caixa.filter({ id: caixaAtivo.id });
            const caixaReal = caixasBanco[0] || caixaAtivo;
            ...
            // Atualizar saldo final do caixa com arredondamento
            const valorDevolucao = Math.round((parseFloat(devolucao.valor_total) || 0) * 100) / 100;
            const novoSaldoFinal = Math.round((parseFloat(caixaReal.saldo_final || caixaReal.saldo_atual || 0) - valorDevolucao) * 100) / 100;
            
            await base44.entities.Caixa.update(caixaAtivo.id, { 
              saldo_final: novoSaldoFinal,
              saldo_atual: novoSaldoFinal // Atualiza ambos por compatibilidade
            });
          }
        } catch (e) {
