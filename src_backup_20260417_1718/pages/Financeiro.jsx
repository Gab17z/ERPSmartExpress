...
export default function Financeiro() {
  ...
  // Mapear devoluções aprovadas por venda para dedução
  ...
  const dadosFinanceiros = useMemo(() => {
    ...
    const faturamentoBruto = vendasPeriodo.reduce((sum, v) => {
      const subtotal = v.subtotal !== undefined ? v.subtotal : ((v.valor_total || 0) + (v.desconto_total || v.desconto || 0));
      return sum + subtotal;
    }, 0);
    const descontos = vendasPeriodo.reduce((sum, v) => sum + (v.desconto_total || v.desconto || 0), 0);
    
    // Deduzir devoluções do faturamento líquido para igualar ao Relatório
    const faturamentoLiquido = vendasPeriodo.reduce((sum, v) => {
      const valorTotal = parseFloat(v.valor_total) || 0;
      const valorDevolvido = devolucoesPorVenda[v.id] || 0;
      return sum + Math.max(0, valorTotal - valorDevolvido);
    }, 0);

    // Contas a receber — inclui parciais; saldo = valor - valor_pago
    const receberPendente = contasReceber
      .filter(c => c.status !== 'pago')
      .reduce((sum, c) => sum + Math.max(0, (parseFloat(c.valor) || 0) - (parseFloat(c.valor_pago) || 0)), 0);

    const receberVencido = contasReceber
      .filter(c => c.status !== 'pago' && new Date(c.data_vencimento) < hoje)
      .reduce((sum, c) => sum + Math.max(0, (parseFloat(c.valor) || 0) - (parseFloat(c.valor_pago) || 0)), 0);

    const receberHoje = contasReceber
      .filter(c => c.status !== 'pago' && isSameDay(new Date(c.data_vencimento), hoje))
      .reduce((sum, c) => sum + Math.max(0, (parseFloat(c.valor) || 0) - (parseFloat(c.valor_pago) || 0)), 0);

    // Contas a pagar — inclui parciais; saldo = valor - valor_pago
    ...
    // Saldo bancario
    const saldoBancario = contasBancarias.reduce((sum, c) => sum + (parseFloat(c.saldo_atual) || 0), 0);
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const receber = contasReceber
        .filter(c => c.status !== 'pago' && isSameDay(new Date(c.data_vencimento), data))
        .reduce((sum, c) => sum + Math.max(0, (parseFloat(c.valor) || 0) - (parseFloat(c.valor_pago) || 0)), 0);
    ...
    for (let i = diasPeriodo - 1; i >= 0; i--) {
      ...
      const faturamentoDia = vendasDia.reduce((sum, v) => {
        const totalv = parseFloat(v.valor_total) || 0;
        const devolvido = devolucoesPorVenda[v.id] || 0;
        return sum + Math.max(0, totalv - devolvido);
      }, 0);
  ...
  }, [vendas, contasReceber, contasPagar, caixas, contasBancarias, periodo, devolucoesPorVenda]);
