...
export default function Metas() {
  ...
  const performancePorUsuario = usuariosSistema.map(usuario => {
    ...
    // Meta individual: lê meta da loja (configurável em Metas Aprimorado) dividida pelo nº de vendedores
    const metasConfig = (() => { try { return JSON.parse(localStorage.getItem('metas_sistema') || '{}'); } catch { return {}; } })();
    const metaLoja = Math.max(1, metasConfig.vendas_loja || 10000);
    const numVendedores = Math.max(1, usuariosSistema.length);
    const meta = Math.round(metaLoja / numVendedores);
  ...
  const totalGeral = vendasPeriodo.reduce((sum, v) => sum + (parseFloat(v.valor_total) || 0), 0);
